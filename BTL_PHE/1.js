...
function main() {
  ...
  var canvas = document.getElementById('webgl');
	...
	document.onkeydown = function (ev) {console.log(ev); keydown(ev); };

  canvas.onmousedown = function (ev) { console.log(ev); mouseDrag(ev, canvas); };
  canvas.onmousemove = function (ev) { console.log(ev); mouseDrag(ev, canvas); };
  document.onmouseup = function (ev) { console.log(ev); mouseDrag(ev, canvas); };
  canvas.onwheel = function (ev) { console.log(ev); mouseDrag(ev, canvas); };
 ...
 // Set the light color (white)
  ...
  gl.uniform3f(u_AmbientLight, 0.2, 0.2, 0.2);
    ...
    var tick = function()
    {
      ...
      var viewX = Math.cos(currentViewAngle*angleToDegree);  //var currentViewAngle = 270;
      var viewZ = Math.sin(currentViewAngle*angleToDegree);  //var angleToDegree = 2*Math.PI/360;
	  // thiết lập điểm nhìn
      viewMatrix.setLookAt(D * Math.cos(a) * Math.cos(b),
      D * Math.cos(b),
      D * Math.cos(a) * Math.sin(b), 0.0, 0.0, 0.0, 0, 1, 0) // var VIEW_REDUIS = 2;
...
//var D = 2;
      ...
      projMatrix.setPerspective(big, canvas.width/canvas.height, 1, 100);
      
      // Đặt màu rõ ràng và cho phép khử mặt khuất
      gl.clearColor(0.0, 0.0, 0.0, 0.0);
      gl.enable(gl.DEPTH_TEST);

      // Thực hiện bộ đệm chiều sâu và bộ đẹm màu
...
//
  ...
  function drawMoon(gl, n, currentAngle, modelMatrix, projMatrix, viewMatrix, u_ModelMatrix, u_ProjMatrix, u_ViewMatrix)
  {
    ...
    var x = Math.sin(currentAngle*angleToDegree);
    var y = Math.cos(currentAngle*angleToDegree);
    
    modelMatrix.setTranslate(x*revolution, 0, y*revolution);
	modelMatrix.rotate(currentAngle, 0, 1, 0);
    gl.uniformMatrix4fv(u_ModelMatrix, false, modelMatrix.elements);
    gl.uniformMatrix4fv(u_ProjMatrix, false, projMatrix.elements);
    gl.uniformMatrix4fv(u_ViewMatrix, false, viewMatrix.elements);
